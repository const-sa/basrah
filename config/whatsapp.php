...
return [
    ...
    */
    ...
    /*
    | كيف تخرج الرسالة:
    |   after_response — تُرسل فور وصول الصفحة للمتصفح، في الطلب نفسه، فلا
    |                    تحتاج عامل طابور (الاستضافة المشتركة لا تُبقيه يعمل).
    |   queue          — تُرمى في الطابور ويرسلها `queue:work`.
    | الإرسال الجماعي يتجاوز حدّ الفوري فيذهب باقيه إلى الطابور، ويصرّفه
    | المُجدول كل دقيقة (routes/console.php).
    */
    'dispatch' => env('WHATSAPP_DISPATCH', 'after_response'),

    'immediate_limit' => (int) env('WHATSAPP_IMMEDIATE_LIMIT', 20),
