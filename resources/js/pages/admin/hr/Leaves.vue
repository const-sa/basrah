...
const statusClass = (s: string) =>
    ({ pending: 'bg-amber-100 text-amber-700', approved: 'bg-emerald-100 text-emerald-700', rejected: 'bg-red-100 text-red-700', settled: 'bg-slate-200 text-slate-600', cancelled: 'bg-slate-200 text-slate-600' })[s] ??
    'bg-slate-100 text-slate-700';
</script>

<template>
    <Head title="الإجازات والسلف" />

    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="min-h-full space-y-4 bg-slate-100 p-5">
            <div class="flex flex-wrap items-center justify-between gap-3">
                <div>
                    <h1 class="text-2xl font-extrabold text-slate-900">الإجازات والسلف</h1>
                    <p class="mt-1 text-sm font-medium text-slate-600">السلفة المعتمدة تُستقطع تلقائيًا من مسيّر الراتب</p>
                </div>
            ...
            <div class="flex gap-1.5">
                ...
                <button
                    v-if="can('leaves.create')" type="button"
                    @click="tab === 'leaves' ? (showLeave = true) : (showAdvance = true)"
                ...
                >
                    <Plus class="h-4 w-4" /> {{ tab === 'leaves' ? 'إجازة جديدة' : 'سلفة جديدة' }}
                </button>
            </div>
