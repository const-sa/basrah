...
const form = useForm({
    ...
    items: props.quotation ? props.quotation.items.map(i => ({
        item_id: i.item_id,
        item: i.item,
        quantity: i.quantity,
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="mx-auto w-full space-y-4 p-4 sm:p-6">
            <div class="flex items-center justify-between">
                ...
                <Link href="/admin/quotations" class="text-sm font-bold text-slate-500 hover:text-slate-800">
                    العودة للسجل
                </Link>
            </div>
                ...
                <div class="grid gap-6 rounded-2xl border-2 border-slate-300 bg-white p-5 shadow-sm sm:grid-cols-2 lg:grid-cols-3">
                    <div>
                        ...
                        <AsyncSelect
                            ...
                            :initial-option="quotation?.client_id ? { id: quotation.client_id, name: clients.find(c => c.id === quotation?.client_id)?.name } : null"
                            required
                        />
                    ...
                    <div>
                        ...
                        <select v-model="form.department_id" required class="w-full rounded-xl border-2 border-slate-300 px-4 py-3 text-sm font-bold shadow-sm focus:border-emerald-700 focus:ring-emerald-200 transition-all">
                    ...
                    <div>
                        ...
                        <input v-model="form.valid_until" type="date" class="w-full rounded-xl border-2 border-slate-300 px-4 py-3 text-sm font-bold shadow-sm focus:border-emerald-700" />
                        ...
                        <table class="w-full text-sm">
                            <thead class="bg-slate-50">
                                <tr>
                                    ...
                                    <th class="px-3 py-2 text-center font-extrabold text-[#064e3b] w-32">الكمية</th>
                                    <th class="px-3 py-2 text-center font-extrabold text-[#064e3b] w-32">السعر</th>
                                    <th class="px-3 py-2 text-center font-extrabold text-[#064e3b] w-32">الضريبة</th>
                                    <th class="px-3 py-2 text-left font-extrabold text-[#064e3b] w-32">الإجمالي</th>
                                    <th class="px-3 py-2 text-center font-extrabold text-[#064e3b] w-12"></th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr v-for="(line, index) in form.items" :key="index" class="border-t border-slate-100">
                                    ...
                                    <td class="px-3 py-2">
                                        <input v-model.number="line.quantity" type="number" min="0.001" step="any" class="w-full rounded-lg border-2 border-slate-300 px-2 py-1.5 text-center text-sm" required />
                                    </td>
                                    <td class="px-3 py-2">
                                        <input v-model.number="line.unit_price" type="number" min="0" step="0.01" class="w-full rounded-lg border-2 border-slate-300 px-2 py-1.5 text-center text-sm" required />
                                    </td>
                                    <td class="px-3 py-2 text-center text-xs font-bold text-slate-500" dir="ltr">
                                    ...
                                    <td class="px-3 py-2 text-center">
                                        <button type="button" @click="removeItem(index)" class="text-slate-400 hover:text-red-600"><Trash2 class="h-4 w-4" /></button>
                                    </td>
                                </tr>
                                <tr v-if="!form.items.length">
                                    <td colspan="6" class="py-8 text-center text-sm text-slate-500">لا يوجد بنود في العرض. انقر "إضافة صنف" للبدء.</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                    ...
                    <div class="rounded-2xl border-2 border-slate-300 bg-slate-50 p-5 shadow-sm">
                        <dl class="space-y-3 text-sm">
                            ...
                            <div class="flex items-center justify-between border-t border-slate-200 pt-3">
                                ...
                                <dd class="w-32">
                                    <input v-model.number="form.discount_amount" type="number" min="0" :max="subtotal" step="0.01" class="w-full rounded-lg border border-slate-300 px-2 py-1.5 text-center text-sm text-red-600 font-bold" />
                                </dd>
                            </div>
