...
import { CheckCheck, ChevronDown, Lock, Pencil, Plus, ShieldCheck, Trash2, Users, X } from 'lucide-vue-next';
...
const breadcrumbs = computed<BreadcrumbItem[]>(() => [
    ...
    { title: t('nav.roles'), href: '/admin/roles' },
]);
...
// ── مستوى النظام: منح أو سحب نظام كامل بضغطة واحدة ──────────
...
// طيّ الأنظمة غير المفعّلة لتقصير النموذج.
...
// ملخص الأنظمة التي يصل إليها الدور — يُعرض على البطاقة بدل سرد كل صلاحية.
...
const submit = () => {
    ...
    if (editingId.value) {
        form.put(`/admin/roles/${editingId.value}`, opts);
    } else {
        form.post('/admin/roles', opts);
    }
};
                    ...
                    <div class="flex flex-1 flex-col p-5">
                        <div class="mb-3 flex items-start justify-between">
                            <div class="flex items-center gap-3">
                                <span class="flex h-12 w-12 items-center justify-center rounded-2xl brand-gradient text-white shadow-md ring-4 ring-emerald-50"><ShieldCheck class="h-6 w-6" /></span>
                                <div>
                    ...
                    <div class="min-h-0 flex-1 space-y-4 overflow-y-auto px-6 py-4">
                        ...
                        <div v-else>
                            ...
                            <div class="space-y-2.5">
                              <div v-for="s in systems" :key="s.key" class="overflow-hidden rounded-2xl border shadow-sm transition" :class="systemSomeOn(s) ? 'border-emerald-200' : 'border-slate-200'">
                                <!-- رأس النظام: منح أو سحب النظام كاملاً -->
                                    ...
                                    <tbody>
                                        <tr v-for="m in s.modules" :key="m.key" class="group border-b border-slate-100 transition last:border-0 hover:bg-emerald-50/30">
                                            ...
                                            <td v-for="act in actionOrder" :key="act" class="px-2 py-2.5 text-center">
                                                ...
                                                <span v-else class="mx-auto block h-1 w-3 rounded-full bg-slate-100" title="غير متاح لهذه الوحدة"></span>
                                            </td>
