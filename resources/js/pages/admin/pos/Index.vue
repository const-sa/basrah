...
// نسبة الصنف تصل صفرًا من الخادم حين تُطفأ.
 ...
 */
const optionsFor = (line: Line) => props.items.filter((i) => i.id === line.item_id || !chosenIds.value.has(i.id));
...
const stockWarning = (line: Line): string | null => {
    ...
    return line.quantity > item.stock_qty ? `الرصيد المتاح ${item.stock_qty} ${item.unit_label} فقط` : null;
};
...
const selectedMethod = computed(() => props.methods.find((m) => m.id === form.payment_method_id) ?? null);
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="min-h-full space-y-4 bg-slate-100 p-4">
            <form @submit.prevent="submit" class="space-y-4">
                ...
                <div class="rounded-2xl border-2 border-slate-300 bg-white p-4 shadow-sm">
                    <div class="mb-3 flex flex-wrap items-center justify-between gap-3">
                        <h2 class="flex items-center gap-1.5 text-base font-extrabold text-slate-950"><Receipt class="h-5 w-5" /> فاتورة جديدة</h2>
                ...
                <div class="overflow-hidden rounded-2xl border-2 border-slate-300 bg-white shadow-sm">
                    <div class="overflow-x-auto">
                        <table class="w-full min-w-[860px] text-sm">
                            ...
                            <tbody>
                                <tr v-for="(line, i) in lines" :key="line.uid" class="border-t border-slate-300 align-top">
                                            ...
                                            <template #option="{ option }">
                                                <span class="flex items-center justify-between gap-2">
                                                    ...
                                                    <span class="flex shrink-0 items-center gap-1">
                                                        <span class="rounded px-1.5 py-0.5 text-[9px] font-bold" :class="typeClass(option.type)">{{
                                                            option.type_label
                                                        }}</span>
                                    ...
                                    <td class="px-2 py-2 text-center">
                                        <button
                                            ...
                                            @click="removeLine(i)"
                                            title="حذف السطر"
                                            class="rounded-lg p-1.5 text-red-600 transition hover:bg-red-100 hover:text-red-700"
                                        >
                ...
                <div class="grid gap-4 lg:grid-cols-[1fr_340px]">
                    <div class="rounded-2xl border-2 border-slate-300 bg-white p-4 shadow-sm">
                        ...
                        <textarea
                            v-model="form.notes"
                            rows="3"
                            class="w-full rounded-xl border border-slate-300 px-3 py-2.5 text-sm text-slate-900 placeholder:text-slate-500 focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-100"
                        ></textarea>
                        <p v-if="form.errors.lines" class="mt-2 rounded-lg border border-red-200 bg-red-100 px-3 py-2 text-xs font-bold text-red-800">
                            {{ form.errors.lines }}
                        </p>
                    </div>
                    ...
                    <div class="rounded-2xl border-2 border-slate-300 bg-white p-4 shadow-sm">
                        <div class="space-y-2 text-sm">
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <span class="text-base font-extrabold" dir="ltr" :class="remaining > 0.005 ? 'text-red-700' : 'text-slate-950'">{{
                                    money(remaining)
                                }}</span>
                            </div>
                        ...
                        <p
                            v-if="debtOnWalkIn"
                            class="mt-3 rounded-lg border border-amber-300 bg-amber-100 px-3 py-2 text-[11px] font-bold text-amber-900"
                        >
            ...
            <div v-if="recentSales.length" class="rounded-2xl border-2 border-slate-300 bg-white p-3 shadow-sm">
                <h3 class="mb-2 flex items-center gap-1.5 text-sm font-extrabold text-slate-800"><Receipt class="h-4 w-4" /> آخر الفواتير</h3>
