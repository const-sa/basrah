...
import { Eye, Pencil, Printer, Receipt, Search, Trash2, X } from 'lucide-vue-next';
...
interface PurchaseRow {
    ...
    /** جواب الفاتورة نفسها: أهي بضريبة أم بدونها. */
    is_taxable: boolean;
    total: number;
    paid: number;
    remaining: number;
...
// السجل يعرض أوراقًا صادرة، فعمود الضريبة يبقى ما دام في المعروض ضريبةٌ
// حُصّلت، ويختفي حين تُطفأ الضريبة ولا يبقى منها شيء.
const { shows } = useVat();

const showsTax = computed(() => shows(props.stats.tax_total));

/** ضريبة ورقةٍ بعينها: جوابها هي أوّلًا، ثم أن يكون للضريبة موضعٌ على الشاشة. */
const rowShowsTax = (row: { is_taxable: boolean; tax_amount: number }) => row.is_taxable && shows(row.tax_amount);
...
const apply = () => router.get('/admin/purchases', filters.value, { preserveState: true, preserveScroll: true, replace: true });
        ...
        <div class="min-h-full space-y-4 bg-slate-100 p-5">
            <div class="flex flex-wrap items-center justify-between gap-3">
                <div>
                    ...
                    <p class="mt-1 text-sm font-medium text-slate-600">فواتير المشتريات ومبالغها ومواردها</p>
                </div>
                <Link
            ...
            <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
                ...
                <StatPill v-if="showsTax" label="الضريبة" :value="money(stats.tax_total)" />
                        ...
                        <tbody>
                            <tr v-for="p in purchases.data" :key="p.id" class="border-t border-slate-100 hover:bg-slate-50">
                                ...
                                <td
                                    ...
                                    :class="p.tax_amount > 0 ? 'text-slate-600' : 'text-slate-400'"
                                    dir="ltr"
                                >
                                ...
                                <td
                                    ...
                                    :class="p.remaining > 0 ? 'text-red-600' : 'text-slate-400'"
                                    dir="ltr"
                                >
                            ...
                            <tr v-if="!purchases.data.length">
                                <td :colspan="showsTax ? 9 : 8" class="px-4 py-10 text-center text-sm text-slate-500">لا فواتير مطابقة للتصفية</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                            ...
                            <dl class="order-1 space-y-1 text-xs sm:order-2">
                                <div class="flex justify-between border-b border-slate-100 py-1">
                                    <dt class="font-bold text-slate-600">
                                        {{ rowShowsTax(details.purchase) ? 'الإجمالي قبل الضريبة' : 'الإجمالي' }}
                                    </dt>
                                ...
                                <div v-if="rowShowsTax(details.purchase)" class="flex justify-between border-b border-slate-100 py-1">
                                ...
                                <div class="flex justify-between bg-slate-800 px-2 py-1.5 text-sm text-white">
                                    <dt class="font-extrabold">{{ rowShowsTax(details.purchase) ? 'الإجمالي شامل الضريبة' : 'الإجمالي' }}</dt>
