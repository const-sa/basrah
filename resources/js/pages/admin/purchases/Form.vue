...
const form = useForm({
    ...
    items: props.purchase
        ? props.purchase.items.map((i) => ({
              item_id: i.item_id,
              item: i.item,
              quantity: i.quantity,
...
// Calculations
// نسبة الصنف تصل من الخادم صفرًا حين يُطفأ مفتاح المنشأة، فيخرج السطر بلا
// ضريبة بلا شرطٍ هنا. وجواب الفاتورة «بدون ضريبة» يصفّرها فوق ذلك.
watch(
    [() => form.items, () => form.discount_amount, () => form.is_taxable],
    ([lines, discount, taxable]) => {
        ...
        <div class="mx-auto w-full space-y-6 p-4 sm:p-6 lg:p-8">
            <div class="flex items-center justify-between border-b border-slate-200 pb-4">
                ...
                <Link
                    ...
                    class="inline-flex items-center justify-center rounded-xl bg-white px-4 py-2 text-sm font-bold text-slate-700 shadow-sm ring-1 ring-inset ring-slate-300 transition-all hover:bg-slate-50"
                >
                    العودة للسجل
                </Link>
            </div>
            ...
            <form @submit.prevent="submit" class="space-y-4">
                    ...
                    <div class="grid gap-5 p-5 sm:grid-cols-2 lg:grid-cols-3">
                        <div>
                            ...
                            <AsyncSelect
                                ...
                                required
                            />
                            <p v-if="form.errors.supplier_id" class="mt-1.5 text-xs font-medium text-red-500">{{ form.errors.supplier_id }}</p>
                        </div>

                        <div>
                            <label class="mb-1.5 flex min-h-[24px] items-center text-sm font-bold text-slate-700"
                                >القسم الوجهة (المستودع) <span class="text-red-500">&nbsp;*</span></label
                            >
                            <select
                                v-model="form.department_id"
                                class="w-full rounded-xl border-slate-200 px-4 py-2.5 text-sm shadow-sm transition-colors focus:border-emerald-600 focus:ring-emerald-600"
                                required
                            >
                                <option v-for="d in departments" :key="d.id" :value="d.id">{{ d.name }}</option>
                            </select>
                            <p v-if="form.errors.department_id" class="mt-1.5 text-xs font-medium text-red-500">{{ form.errors.department_id }}</p>
                        </div>
                        ...
                        <div>
                            ...
                            <p v-if="form.errors.payment_method_id" class="mt-1.5 text-xs font-medium text-red-500">
                        ...
                        <div>
                            ...
                            <p v-if="form.errors.is_taxable" class="mt-1.5 text-xs font-medium text-red-500">{{ form.errors.is_taxable }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="relative z-50 rounded-2xl border-2 border-slate-300 bg-white shadow-sm transition-all duration-200 hover:shadow-md">
                    <div class="flex items-center justify-between rounded-t-2xl border-b border-slate-200 bg-slate-50 px-5 py-3">
                        <h2 class="flex items-center gap-2 text-sm font-bold text-slate-800">
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm">
                            <thead class="bg-slate-50">
                                <tr>
                                    <th class="bg-emerald-100/50 px-4 py-3 text-right font-extrabold text-[#064e3b]">الصنف</th>
                                    <th class="w-32 bg-emerald-100/50 px-4 py-3 text-center font-extrabold text-[#064e3b]">الكمية</th>
                                    <th class="w-36 bg-emerald-100/50 px-4 py-3 text-center font-extrabold text-[#064e3b]">تكلفة الوحدة</th>
                                    <th v-if="vatApplies" class="w-32 bg-emerald-100/50 px-4 py-3 text-center font-extrabold text-[#064e3b]">
                                        الضريبة
                                    </th>
                                    <th class="w-36 bg-emerald-100/50 px-4 py-3 text-left font-extrabold text-[#064e3b]">الإجمالي</th>
                                    <th class="w-12 bg-emerald-100/50 px-4 py-3 text-center font-extrabold text-[#064e3b]"></th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr
                                ...
                                >
                                    ...
                                    <td class="px-4 py-3">
                                        <input
                                            ...
                                            min="0.001"
                                            step="any"
                                            class="w-full rounded-lg border-slate-200 px-3 py-1.5 text-center text-sm shadow-sm focus:border-emerald-600 focus:ring-emerald-600"
                                            required
                                        />
                                    </td>
                                    <td class="px-4 py-3">
                                    ...
                                    <td
                                        v-if="vatApplies"
                                        class="px-4 py-3 text-center text-xs font-bold"
                                        :class="form.is_taxable ? 'text-slate-500' : 'text-slate-300'"
                                        dir="ltr"
                                    >
                                ...
                                <tr v-if="!form.items.length">
                                    <td :colspan="vatApplies ? 6 : 5" class="py-12 text-center">
                                        <div class="flex flex-col items-center justify-center text-slate-400">
                                            <svg class="mb-3 h-12 w-12 text-slate-200" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                <path
                                            ...
                                            <p class="mt-1 text-xs">انقر "إضافة صنف" للبدء بإدراج المشتريات.</p>
                                        </div>
                                    </td>
                                </tr>
                            </tbody>
                    ...
                    <p v-for="(err, key) in form.errors" :key="key" class="text-left text-xs text-red-500" dir="ltr">
                ...
                <div class="grid gap-6 sm:grid-cols-12">
                    <div class="sm:col-span-7">
                        <div class="h-full overflow-hidden rounded-2xl border-2 border-slate-300 bg-white shadow-sm">
                            <div class="border-b border-slate-200 bg-slate-50 px-5 py-3">
                                <h2 class="flex items-center gap-2 text-sm font-bold text-slate-800">
                            ...
                            <div class="p-5">
                                <textarea
                                    v-model="form.notes"
                                    rows="6"
                                    class="w-full rounded-xl border-slate-200 px-4 py-3 text-sm shadow-sm transition-colors focus:border-emerald-600 focus:ring-emerald-600"
                                    placeholder="أي تفاصيل أو شروط إضافية تخص هذه المشتريات..."
                                ></textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="sm:col-span-5">
                        <div class="relative overflow-hidden rounded-2xl border border-emerald-900 bg-emerald-950 text-white shadow-lg">
                            <!-- Background decoration -->
                            <div class="pointer-events-none absolute left-0 top-0 h-32 w-32 rounded-br-full bg-emerald-600/10"></div>

                            <div class="border-b border-emerald-900 px-6 py-4">
                                ...
                                <dl class="relative z-10 space-y-4 text-sm">
                                    <div class="flex items-center justify-between text-slate-300">
                                        <dt class="font-medium">
                                            {{ vatApplies && form.is_taxable ? 'الإجمالي قبل الضريبة' : 'إجمالي الأصناف' }}
                                        </dt>
                                        <dd class="font-bold tracking-wide text-white" dir="ltr">{{ money(subtotal) }}</dd>
                                    </div>
                                    <div class="flex items-center justify-between border-t border-emerald-900 pt-4">
                                        ...
                                        <dd class="w-32">
                                            <input
                                                ...
                                                :max="subtotal"
                                                step="0.01"
                                                class="w-full rounded-lg border-slate-700 bg-slate-800/50 px-3 py-2 text-center text-sm font-bold text-red-400 placeholder-slate-600 transition-colors focus:border-red-500 focus:ring-red-500"
                                            />
                                        </dd>
                                    </div>
                                    <div v-if="vatApplies" class="flex items-center justify-between border-t border-emerald-900 pt-4 text-slate-300">
                                        <dt class="font-medium">الضريبة</dt>
                                        <dd v-if="form.is_taxable" class="font-bold tracking-wide text-white" dir="ltr">{{ money(totalTax) }}</dd>
                                        <dd v-else class="text-xs font-bold text-slate-500">فاتورة بدون ضريبة</dd>
                                    </div>
                                    <div
                                        class="flex items-center justify-between rounded-xl border border-emerald-600/30 bg-emerald-700/20 px-4 py-3 text-emerald-200"
                                    >
                                        <dt class="font-bold">الإجمالي المستحق</dt>
                                        <dd class="text-base font-extrabold tracking-wide" dir="ltr">{{ money(grandTotal) }}</dd>
                                    </div>
                                    <div class="flex items-center justify-between border-t border-emerald-900 pt-4">
                                        ...
                                        <dd class="w-32">
                                            <input
                                                ...
                                                :max="grandTotal"
                                                step="0.01"
                                                class="w-full rounded-lg border-slate-700 bg-slate-800/50 px-3 py-2 text-center text-sm font-bold text-emerald-400 placeholder-slate-600 transition-colors focus:border-emerald-500 focus:ring-emerald-500"
                                            />
                                        </dd>
                                    </div>
                                    <div class="flex items-center justify-between pt-2">
                                        ...
                                        <dd
                                            ...
                                            :class="grandTotal - form.paid_amount > 0 ? 'text-red-400' : 'text-slate-400'"
                                            dir="ltr"
                                        >
