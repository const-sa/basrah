...
interface SaleRow {
    ...
    can_settle: boolean; can_refund: boolean;
}

interface SaleLine {
    id: number; item_id: number; name: string; code: string | null;
    quantity: number; returned_quantity: number; returnable_quantity: number;
    unit_price: number; discount_amount: number; tax_amount: number; total: number;
}

interface SaleDetails {
    sale: SaleRow & {
        notes: string | null; subtotal: number; discount_amount: number;
        original: { id: number; number: string } | null;
    };
    lines: SaleLine[];
    returns: { id: number; number: string; date: string; total: number; notes: string | null }[];
    vouchers: {
        id: number; number: string; date: string; amount: number;
        treasury: string | null; method_label: string; status: string; status_label: string;
    ...
    issuer: {
        ...
        activity: string | null; qr: string | null;
    };
}
...
const props = defineProps<{
    ...
    stats: {
        count: number; sales_total: number; tax_total: number; returns_total: number;
        collected: number; remaining: number; partial_count: number; unpaid_count: number;
    };
...
const apply = () =>
    router.get('/admin/sales', filters.value, { preserveState: true, preserveScroll: true, replace: true });
...
const refundTotal = computed(() =>
    refundLines.value.reduce((sum, l) => sum + (refundForm.quantities[l.item_id] ?? 0) * l.unit_price, 0),
);
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="min-h-full space-y-4 bg-slate-100 p-5">
            <div class="flex flex-wrap items-center justify-between gap-3">
                <div>
                    ...
                    <p class="mt-1 text-sm font-medium text-slate-600">
                        فواتير القسم ومرتجعاتها وسندات قبضها — والمتبقي على كل فاتورة مسدّدة جزئيًا
                    </p>
                </div>
                <Link v-if="can('pos.create')" href="/admin/pos" class="inline-flex items-center gap-1.5 rounded-md bg-blue-600 px-4 py-2 text-sm font-bold text-white hover:bg-blue-700">
            ...
            <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-6">
                ...
                <StatPill label="الضريبة" :value="money(stats.tax_total)" variant="info" />
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody>
                            <tr v-for="s in sales.data" :key="s.id" class="border-t border-slate-100 hover:bg-slate-50">
                                ...
                                <td class="px-4 py-3 text-left text-xs font-bold" :class="s.tax_amount > 0 ? 'text-slate-600' : 'text-slate-400'" dir="ltr">
                                    {{ s.tax_amount > 0 ? money(s.tax_amount) : '—' }}
                                </td>
                                <td class="px-4 py-3 text-left font-extrabold text-slate-800" dir="ltr">{{ money(s.total) }}</td>
                                <td class="px-4 py-3 text-left text-xs font-bold" :class="s.returned > 0 ? 'text-red-600' : 'text-slate-400'" dir="ltr">
                                    {{ s.returned > 0 ? money(s.returned) : '—' }}
                                </td>
                                <td class="px-4 py-3 text-left text-xs font-bold text-emerald-700" dir="ltr">{{ money(s.paid) }}</td>
                                <td class="px-4 py-3 text-left text-xs font-extrabold" :class="s.remaining > 0 ? 'text-red-600' : 'text-slate-400'" dir="ltr">
                                    {{ s.remaining > 0 ? money(s.remaining) : '—' }}
                                </td>
                                <td class="px-4 py-3 text-center">
                                    <span v-if="s.type === 'sale'" class="rounded-md px-2 py-0.5 text-[11px] font-bold" :class="statusClass(s.payment_status)">
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center justify-center gap-1">
                                        <button type="button" @click="openDetails(s)" title="تفاصيل الفاتورة" class="rounded-lg bg-slate-500 p-1.5 text-white hover:bg-slate-600">
                                            <Eye class="h-3.5 w-3.5" />
                                        </button>
                                        <button v-if="can('sales.create') && s.can_settle" type="button" @click="openSettle(s)" title="سند قبض" class="rounded-lg bg-emerald-500 p-1.5 text-white hover:bg-emerald-600">
                                            <Wallet class="h-3.5 w-3.5" />
                                        </button>
                                        <button v-if="can('sales.create') && s.can_refund" type="button" @click="openRefund(s)" title="مرتجع" class="rounded-lg bg-amber-500 p-1.5 text-white hover:bg-amber-600">
                            ...
                            <tr v-if="!sales.data.length">
                                <td colspan="11" class="px-4 py-10 text-center text-sm text-slate-500">لا فواتير مطابقة للتصفية</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
        ...
        -->
        <Teleport to="body">
        <div v-if="activeSale" class="invoice-overlay fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/40 p-4" @click.self="closeDetails">
            <div class="invoice-sheet my-auto w-full max-w-3xl rounded-2xl bg-white shadow-2xl">
                <div class="flex items-center justify-between gap-2 border-b border-slate-100 px-6 py-3 print:hidden">
                    <div class="flex gap-2">
                        <button type="button" @click="printInvoice" class="inline-flex items-center gap-1.5 rounded-md bg-blue-600 px-4 py-2 text-sm font-bold text-white hover:bg-blue-700">
                            <Printer class="h-4 w-4" /> طباعة
                        </button>
                        <button v-if="can('sales.create') && activeSale.can_settle" type="button" @click="openSettle(activeSale)" class="rounded-md bg-emerald-600 px-4 py-2 text-sm font-bold text-white hover:bg-emerald-700">سند قبض</button>
                        <button v-if="can('sales.create') && activeSale.can_refund" type="button" @click="openRefund(activeSale)" class="rounded-md bg-amber-500 px-4 py-2 text-sm font-bold text-white hover:bg-amber-600">مرتجع</button>
                    </div>
                ...
                <div v-else-if="details" class="invoice-body space-y-5 px-8 py-6">
                    ...
                    <header class="invoice-head grid gap-6 border-b-2 border-slate-800 pb-4 sm:grid-cols-2">
                        ...
                        <div class="text-left">
                            <div class="mb-2 inline-block rounded-lg bg-slate-800 px-3 py-1 text-sm font-extrabold text-white">
                                {{ details.sale.type === 'return' ? 'إشعار دائن (مرتجع)' : 'فاتورة ضريبية مبسّطة' }}
                            </div>
                    ...
                    <table class="w-full border border-slate-300 text-xs">
                        ...
                        <tbody>
                            <tr v-for="(l, i) in details.lines" :key="l.id">
                                ...
                                <td class="border border-slate-300 px-2 py-1.5 text-center" dir="ltr">{{ money(l.tax_amount) }}</td>
                    ...
                    <div v-if="details.returns.length || details.vouchers.length" class="grid gap-4 border-t border-slate-200 pt-4 sm:grid-cols-2">
                        <div v-if="details.returns.length">
                            <h3 class="mb-1.5 text-xs font-extrabold text-slate-800">المرتجعات</h3>
                            <ul class="space-y-1">
                                <li v-for="r in details.returns" :key="r.id" class="flex items-center justify-between gap-2 border-b border-slate-100 py-1 text-[11px]">
                        ...
                        <div v-if="details.vouchers.length">
                            ...
                            <ul class="space-y-1">
                                <li v-for="v in details.vouchers" :key="v.id" class="flex items-center justify-between gap-2 border-b border-slate-100 py-1 text-[11px]">
                                    ...
                                    <span class="font-extrabold" :class="v.status === 'cancelled' ? 'text-slate-400 line-through' : 'text-emerald-700'" dir="ltr">
                                        {{ money(v.amount) }}
                                    </span>
                                </li>
                            </ul>
            ...
            <div class="w-full max-w-md rounded-2xl bg-white shadow-2xl">
                ...
                <form @submit.prevent="submitSettle" class="space-y-3 px-6 py-4">
                    <div>
                        ...
                        <input v-model.number="settleForm.amount" type="number" min="0.01" :max="settleSale.remaining" step="0.01" class="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm" />
                    ...
                    <div>
                        ...
                        <input v-model="settleForm.voucher_date" type="date" class="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm" />
                    </div>

                    <div>
                        <label class="mb-1 block text-sm font-bold text-slate-700">البيان</label>
                        <textarea v-model="settleForm.description" rows="2" class="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm"></textarea>
                    </div>
                    ...
                    <div class="flex justify-end gap-2 border-t border-slate-100 pt-3">
                        <button type="button" @click="settleSale = null" class="rounded-xl border border-slate-200 px-4 py-2 text-sm font-bold text-slate-600">إلغاء</button>
                        <button type="submit" :disabled="settleForm.processing" class="rounded-md bg-emerald-600 px-5 py-2 text-sm font-bold text-white hover:bg-emerald-700 disabled:opacity-60">حفظ وترحيل</button>
                    </div>
                </form>
            </div>
        </div>
            ...
            <div class="flex max-h-[92vh] w-full max-w-xl flex-col rounded-2xl bg-white shadow-2xl">
                <div class="flex items-center justify-between border-b border-slate-100 px-6 py-4">
                    <div>
                        ...
                        <p class="text-xs font-medium text-slate-500">من الفاتورة <span dir="ltr">{{ refundSale.number }}</span></p>
                    </div>
                ...
                <form @submit.prevent="submitRefund" class="flex min-h-0 flex-1 flex-col">
                    <div class="min-h-0 flex-1 space-y-3 overflow-y-auto px-6 py-4">
                        ...
                        <p v-else-if="!refundLines.length" class="rounded-xl bg-slate-50 px-3 py-3 text-sm text-slate-500">لا كميات متاحة للإرجاع في هذه الفاتورة.</p>

                        <template v-else>
                            <div class="flex justify-end">
                                <button type="button" @click="fillRefund" class="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-bold text-slate-600 hover:bg-slate-50">إرجاع الكل</button>
                            </div>
                                ...
                                <table class="w-full text-xs">
                                    ...
                                    <tbody>
                                        <tr v-for="l in refundLines" :key="l.id" class="border-t border-slate-100">
                                            ...
                                            <td class="px-3 py-2">
                                                <input
                                                    v-model.number="refundForm.quantities[l.item_id]" type="number"
                                                    min="0" :max="l.returnable_quantity" step="0.001"
                            ...
                            <div>
                                ...
                                <textarea v-model="refundForm.reason" rows="2" class="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm"></textarea>
                            </div>
                            ...
                            <div class="flex items-center justify-between rounded-xl bg-amber-50 px-3 py-2 text-sm">
                                <span class="font-bold text-amber-800">قيمة المرتجع قبل الضريبة</span>
                    ...
                    <div class="flex justify-end gap-2 border-t border-slate-100 px-6 py-4">
                        <button type="button" @click="refundSale = null" class="rounded-xl border border-slate-200 px-4 py-2 text-sm font-bold text-slate-600">إلغاء</button>
                        <button type="submit" :disabled="refundForm.processing || !refundLines.length || refundTotal <= 0" class="rounded-md bg-amber-500 px-5 py-2 text-sm font-bold text-white hover:bg-amber-600 disabled:opacity-60">
                            تأكيد المرتجع
                        </button>
                    </div>
                </form>
