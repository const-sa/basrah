...
const form = useForm({
    ...
    type: 'stock',
    unit: 'piece',
    measure_unit_id: null as number | null,
    cost: 0,
    price: 0,
    tax_rate: 15,
    stock_qty: 0,
...
const submitStocktake = () => {
    stocktake
        .transform((d) => ({
            ...
            adjustments: d.adjustments.filter((a) => a.counted_qty !== a.current).map((a) => ({ item_id: a.item_id, counted_qty: a.counted_qty })),
        }))
        ...
        <div class="min-h-full space-y-4 bg-slate-100 p-5">
            <div class="flex flex-wrap items-center justify-between gap-3">
                ...
                <div class="flex flex-wrap gap-2">
                    ...
                    <button
                        v-if="can('inventory.approve')"
                        type="button"
                        @click="openStocktake"
                        class="inline-flex items-center gap-1.5 rounded-md bg-amber-500 px-4 py-2 text-sm font-bold text-white hover:bg-amber-600"
                    >
                ...
                <div class="grid gap-2 sm:grid-cols-2 lg:grid-cols-5">
                    <div class="relative">
                        ...
                        <input
                            ...
                            placeholder="اسم أو كود أو باركود"
                    ...
                    <label
                        class="flex cursor-pointer items-center gap-2 rounded-xl border border-slate-200 px-3 py-2.5 text-sm font-bold text-slate-700"
                    >
                        <input
                            type="checkbox"
                        ...
                        <tbody>
                            ...
                            >
                                ...
                                <td class="px-4 py-3 text-center">
                                    <span v-if="i.department" class="rounded-md bg-indigo-100 px-2 py-0.5 text-[11px] font-bold text-indigo-700">{{
                                        i.department
                                    }}</span>
                                ...
                                <td class="px-4 py-3 text-center">
                                    <template v-if="i.tracks_stock">
                                        ...
                                        <span
                                            v-if="i.low_stock"
                                            class="mt-0.5 inline-flex items-center gap-0.5 rounded bg-red-100 px-1.5 text-[10px] font-bold text-red-700"
                                        >
                            ...
                            <tr v-if="!items.data.length">
                                <td colspan="6" class="px-4 py-10 text-center text-sm text-slate-500">لا أصناف</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                ...
                <form @submit.prevent="submit" class="flex min-h-0 flex-1 flex-col">
                    <div class="min-h-0 flex-1 space-y-3 overflow-y-auto px-6 py-4">
                        ...
                        <div class="grid gap-3 sm:grid-cols-3">
                            ...
                            <div>
                                ...
                                <select v-model="form.measure_unit_id" class="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm">
                                    ...
                                    <option v-for="u in measureUnits" :key="u.id" :value="u.id">
                        ...
                        <div class="grid gap-3 sm:grid-cols-4">
                            ...
                            <!--
                                نسبة الصنف تُحفظ ولا تُعرض ما دامت الضريبة مطفأة: حقلٌ
                                يُملأ ولا يُحتسب منه شيء يوهم الموظف أنه سعّر ضريبةً.
                                وتعود بقيمتها المحفوظة متى فُعّلت.
                            -->
                            <div v-if="vatApplies">
                        ...
                        <div v-if="isBundle" class="rounded-xl border border-amber-200 bg-amber-50 p-3">
                            <div class="mb-2 flex items-center justify-between">
                                ...
                                <button
                                    type="button"
                                    @click="addComponent"
                                    class="rounded-lg bg-white px-2 py-1 text-[11px] font-bold text-amber-700 ring-1 ring-amber-200"
                                >
                                    + مكوّن
                                </button>
                            </div>
                            <div class="space-y-1.5">
                                <div v-for="(c, i) in form.components" :key="i" class="flex items-center gap-2">
                                    ...
                                    />
                                    <button type="button" @click="removeComponent(i)" class="text-red-500 hover:text-red-700">
                                        <Trash2 class="h-3.5 w-3.5" />
                                    </button>
                                </div>
                                <p v-if="!form.components.length" class="text-[11px] text-amber-700">
                                    أضف المكوّنات وإلا لن تُخصم أي كمية عند بيع الحزمة.
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex justify-end gap-2 border-t border-slate-100 px-6 py-4">
                        ...
                        <button
                            ...
                            class="rounded-md bg-blue-600 px-5 py-2 text-sm font-bold text-white hover:bg-blue-700 disabled:opacity-60"
                        >
                            حفظ
                        </button>
                    </div>
            ...
            <div class="flex max-h-[92vh] w-full max-w-2xl flex-col rounded-2xl bg-white shadow-2xl">
                ...
                <form @submit.prevent="submitStocktake" class="flex min-h-0 flex-1 flex-col">
                        ...
                        <table class="w-full text-sm">
                            ...
                            <tbody>
                                <tr v-for="a in stocktake.adjustments" :key="a.item_id" class="border-t border-slate-100">
                                    ...
                                    <td
                                        class="px-2 py-1.5 text-center text-xs font-bold"
                                        :class="
                                            a.counted_qty === a.current
                                                ? 'text-slate-400'
                                                : a.counted_qty > a.current
                                                  ? 'text-emerald-600'
                                                  : 'text-red-600'
                                    ...
                                    >
                                        {{
                                            a.counted_qty === a.current
                                                ? '—'
                                                : (a.counted_qty > a.current ? '+' : '') + (a.counted_qty - a.current).toFixed(3)
                                        }}
                                    </td>
                                </tr>
                            </tbody>
                    ...
                    <div class="flex items-center justify-between gap-2 border-t border-slate-100 px-6 py-4">
                        <span class="text-xs font-bold" :class="changedCount ? 'text-amber-700' : 'text-slate-400'"
                            >{{ changedCount }} صنف مختلف</span
                        >
                        <div class="flex gap-2">
                            <button
                                type="button"
                                @click="showStocktake = false"
                                class="rounded-xl border border-slate-200 px-4 py-2 text-sm font-bold text-slate-600"
                            >
                                إلغاء
                            </button>
                            <button
                                type="submit"
                                :disabled="stocktake.processing || !changedCount"
                                class="rounded-md bg-amber-500 px-5 py-2 text-sm font-bold text-white hover:bg-amber-600 disabled:opacity-50"
                            >
                                اعتماد التسوية
                            </button>
                        </div>
